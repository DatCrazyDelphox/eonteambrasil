...
$data_hora_atual = date("Y-m-d H:i:s");
        ...
        //Historico
	$SQLSelectSSH = "SELECT * FROM usuario_ssh  WHERE online = '0'    ";
    $SQLSelectSSH = $conn->prepare($SQLSelectSSH);
    $SQLSelectSSH->execute();

	 if(($SQLSelectSSH->rowCount()) > 0){
	        while($ssh = $SQLSelectSSH->fetch()){
				$SQLHistSSH = "update hist_usuario_ssh_online set hora_desconexao='".$data."' , status='0'
				                    WHERE status='1' and id_usuario='".$ssh['id_usuario_ssh']."' ";
	...
	$SQLUPSSH = "update usuario_ssh set online_hist='0'  WHERE online = '0'    ";
   ...
   $SQLServidor = "SELECT * FROM servidor where tipo='premium' ";
   ...
   if(($SQLServidor->rowCount()) > 0){
	   ...
	   while($row = $SQLServidor->fetch()){
					 ...
					 $SQLUPSSH = "update usuario_ssh set online='0'  WHERE id_servidor = '".$row['id_servidor']."'    ";
                     $SQLUPSSH = $conn->prepare($SQLUPSSH);
                     $SQLUPSSH->execute();
		   ...
		   for($i = 0;$i <count($Array);$i++){
            ...
            $SQLSSHServidor = "SELECT * FROM usuario_ssh where login='$user' ";
			 ...
			 if(($SQLSSHServidor->rowCount()) > 0){
				 $sssh =  $SQLSSHServidor->fetch();
					 ...
					 if($sssh['online_hist'] == 0){
						$SQLUPSSH = "update usuario_ssh set online='".$Array[$i+1]."' , online_hist='1' ,
                        ...
                        $SQLHistSSH = "INSERT INTO hist_usuario_ssh_online (status, id_usuario, hora_conexao)
                                                VALUES ('1', '".$sssh['id_usuario_ssh']."', '".$data."' )";

                        $SQLHistSSH = $conn->prepare($SQLHistSSH);
                        $SQLHistSSH->execute();

					 }else{
						$SQLUPSSH = "update usuario_ssh set online='".$Array[$i+1]."' , online_hist='1'  WHERE login = '$user'  ";
                        $SQLUPSSH = $conn->prepare($SQLUPSSH);
                        $SQLUPSSH->execute();
					    }
